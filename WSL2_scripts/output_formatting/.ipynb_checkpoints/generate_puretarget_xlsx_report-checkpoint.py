import os
import csv
import sys
import glob

# pip install pandas cairosvg openpyxl (in conda environment)
import pandas as pd
import gzip
import re
import cairosvg
from openpyxl import Workbook
from openpyxl.styles import Font, PatternFill
from openpyxl.drawing.image import Image

# run in bash:
# conda activate bit11
# cd /home/svdsompe/BIT11/output_formatting/bin
# output_dir="/home/svdsompe/BIT11/output_formatting/r84299_20250623_141348"
# samplesheet=$output_dir/samplesheet.csv
# python3 generate_puretarget_xlsx_report.py $output_dir $samplesheet

def read_samplesheet(samplesheet_path):
    with open(samplesheet_path, newline='') as csvfile:
        return list(csv.DictReader(csvfile))
    

def write_report(samples,output_dir):   

    for sample in samples:
        sample_name = sample['sample']
        filepath = os.path.join(output_dir, sample_name)
        vcf_file = os.path.join(filepath, f"{sample_name}.sorted.vcf.gz")
        output_excel = os.path.join(filepath, f"PureTarget_analysis_report_{sample_name}.xlsx")

        # Extract TRGT version from VCF
        with gzip.open(vcf_file, 'rt') as f:
            for line in f:
                if "##trgtVersion=" in line:
                    trgt_version = re.search(r'trgtVersion=(\S+)', line).group(1)
                    trgt_version = trgt_version.split('-')[0]
                    break  # stop loop once found

        # Parse VCF
        vcf_data = []
        with gzip.open(vcf_file, 'rt') as f:
            for line in f:
                if line.startswith("#"):
                    continue
                parts = line.strip().split('\t')
                chrom, pos, _, ref, alt, _, _, info, fmt, smpl = parts
                info_dict = dict([x.split('=') for x in info.split(';')])
                fmt_keys = fmt.split(':')
                smpl_values = smpl.split(':')
                fmt_dict = dict(zip(fmt_keys, smpl_values))
                vcf_data.append({
                    "CHROM": chrom,
                    "POS": pos,
                    "REF": ref,
                    "ALT": alt,
                    "TRID": info_dict.get("TRID"),
                    "MOTIFS": info_dict.get("MOTIFS"),
                    "GT": fmt_dict.get("GT"),
                    "AL": fmt_dict.get("AL"),
                    "ALLR": fmt_dict.get("ALLR"),
                    "SD": fmt_dict.get("SD"),
                    "MC": fmt_dict.get("MC"),
                    "MS": fmt_dict.get("MS"),
                    "AP": fmt_dict.get("AP"),
                    "AM": fmt_dict.get("AM")
        })
        vcf_df = pd.DataFrame(vcf_data)

        # Create Excel workbook
        wb = Workbook()
        ws = wb.active
        ws.title = "report"

        # Write header
        ws.append(["PureTarget analysis report"])
        ws.append([f"{sample_name}"])
        ws.append([f"TRGT v{trgt_version}"])
        ws.append([])
        cell = ws.cell(row=1, column=1)
        cell.font = Font(name='Calibri', size=18, bold=True, color='FFE31B92')
        bold_font = Font(name='Calibri', bold=True)
        grey_fill = PatternFill(start_color='FFD3D3D3', end_color='FFD3D3D3', fill_type='solid')
        for row in ws['A2:A3']:
            for cell in row:
                cell.font = bold_font

        # Write summary per repeat
        for idx, row in vcf_df.iterrows():
            ws.append(["Gene", row['TRID']])
            ws.cell(row=ws.max_row, column=1).font = bold_font
            ws.append(["Position", f"{row['CHROM']}:{row['POS']}"])
            ws.cell(row=ws.max_row, column=1).font = bold_font
            ws.append(["Expected motifs", row['MOTIFS']])
            ws.cell(row=ws.max_row, column=1).font = bold_font
            ws.append(["Genotype", row['GT']])
            ws.cell(row=ws.max_row, column=1).font = bold_font
            for row_idx in range(ws.max_row-3,ws.max_row+1):
                for col_idx in range(1, 100):
                    ws.cell(row=row_idx, column=col_idx).fill = grey_fill
            ws.append([])
            ws.append(["Allele", "Length", "Length range", "Reads spanning allele", "Motif count", "Motif span", "Purity", "Mean methylation", "Sequence"])
            for cell in ws[ws.max_row]:
                cell.font = bold_font

            # Determine alleles based on GT
            gt = row.get("GT", "")
            ref = row["REF"]
            alt_alleles = row["ALT"].split(",") if row["ALT"] else []

            if "/" in gt:
                allele_indices = gt.split("/")
            else:
                # "0" or "1" for chrX
                allele_indices = [gt]       

            # Convert to integers (ignore '.')
            allele_indices = [int(a) for a in allele_indices if a.isdigit()]   

            # Helper to safely split list fields
            def safe_split(value):
                return value.split(",") if value else []

            al_list  = safe_split(row.get("AL"))
            allr_list = safe_split(row.get("ALLR"))
            sd_list  = safe_split(row.get("SD"))
            mc_list  = safe_split(row.get("MC"))
            ms_list  = safe_split(row.get("MS"))
            ap_list  = safe_split(row.get("AP"))
            am_list  = safe_split(row.get("AM"))

            # Build a mapping
                # 0:REF
                # 1:ALT_1
                # 2:ALT_2
            allele_map = {0: ref}
            for i, alt in enumerate(alt_alleles):
                allele_map[i+1] = alt

            # Loop through alleles in GT and write one row per allele
            for idx, allele_index in enumerate(allele_indices):
                allele_seq = allele_map.get(allele_index, "")
                allele_label = f"Allele {idx+1}"            
                row_cells = [
                    allele_label,
                    al_list[idx],
                    allr_list[idx],
                    sd_list[idx],
                    mc_list[idx],
                    ms_list[idx],
                    ap_list[idx],
                    am_list[idx],
                    allele_seq
                ]
                ws.append(row_cells)
            ws.append([])
            ws.append([])
            
        # Final formatting of 'report' sheet
        for col in ws.columns:
            ws.column_dimensions[col[0].column_letter].width = 17

        # For each repeat, print images on separate sheet
        for trid in vcf_df['TRID'].unique():
            # Excel sheet name max 31 chars
            sheet_name = trid[:31]  
            ws_trid = wb.create_sheet(title=sheet_name)
            motif_svg = os.path.join(filepath, f"{sample_name}.{trid}.motifs.svg")
            meth_svg  = os.path.join(filepath, f"{sample_name}.{trid}.meth.svg")
    
            # Define starting location
            row_pos = 1
            col_pos = 1

            for label, cell, svg_path  in [("Motifs", "A2", motif_svg), ("Methylation", "M2", meth_svg)]:
                if os.path.exists(svg_path):
                    png_path = os.path.splitext(svg_path)[0] + ".png"
                    cairosvg.svg2png(url=svg_path, write_to=png_path, output_width=600)

                    # Insert a label
                    ws_trid.cell(row=row_pos, column=col_pos, value=f"{label} {trid}")
                    ws_trid.cell(row=row_pos, column=col_pos).font = bold_font
                    col_pos += 12

                    # Insert image and reset starting location
                    img = Image(png_path)
                    ws_trid.add_image(img, cell)
                    
        # Save workbook
        wb.save(output_excel)
        print(f"Report saved to {output_excel}")

        #Remove png images from output_dir
        for png_file in glob.glob(os.path.join(filepath, "*.png")):
            os.remove(png_file)
            print(f"Removed: {png_file}")

def main():
    if len(sys.argv) != 3:
        print(f"Usage: python generate_puretarget_xlsx_report.py <output_dir> <samplesheet>")
        sys.exit(1)

    output_dir = sys.argv[1]
    samplesheet = sys.argv[2]

    samples = read_samplesheet(samplesheet)
    write_report(samples,output_dir)

if __name__ == "__main__":
    main()